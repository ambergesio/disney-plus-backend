...
const isAdmin = async (req, res, next) => {
    try {
        ...
        if (!token) return res.status(400).json({ error: true, message: 'You must login in order to perform the action.'});

        const user = await verifyToken(token.split(' ')[1]);
        if (!user) return res.status(400).json({ error: true, message: 'Invalid token'});

        const isValidUser = await getUserByEmailRepo(user.email);
        if (!isValidUser) return res.status(400).json({ error: true, message: 'User not found'});
